...
from aadi.assets.pyroplug import get_msg, check, get_bulk_msg
from aadi.assets.functions import get_link, join, screenshot, force_sub
from aadi.assets.login import get_session
import logging
import asyncio
import pymongo
...
@aadi.on(events.NewMessage(incoming=True, pattern='/batch'))
async def _batch(event):
    ...
    async with aadi.conversation(event.chat_id) as conv: 
        try:
            ...
            cd = await conv.send_message("**Batch process ongoing...**\n\nProcess completed: ", 
                                    buttons=[[Button.url("Join Channel", url="https://t.me/+Vtkx55uLT8sxNGE1")]])
...
@aadi.on(events.NewMessage(incoming=True, pattern='/cancel'))
            ...
            if int(fw.value) > 300:
                ...
                try:
                    ...
                    if countdown.text != count_down:
                        await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/+Vtkx55uLT8sxNGE1")]])
        except Exception as e:
            #logger.info(e)
