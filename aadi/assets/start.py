...
from aadi.assets.functions import screenshot
import subprocess
...
@aadi.on(events.NewMessage(pattern=f"^/start"))
async def start(event):
    ...
    collection.update_one({"user_id": user_id}, {"$set": {"user_id": user_id}}, upsert=True)
    buttons = [
        [Button.url("Join Channel", url="https://t.me/SRC2026_1")],
...
@aadi.on(events.NewMessage(pattern=f"^/gcast"))
async def broadcast(event):
    ...
    for user_doc in collection.find():
        try:
            ...
            await aadi.send_message(user_id, message)
...
@aadi.on(events.NewMessage(incoming=True, pattern='/get'))
...
@aadi.on(events.NewMessage(pattern=f"^{M}"))
...
@aadi.on(events.NewMessage(pattern=f"^{T}"))
async def term_command(event):
    # Creating inline keyboard with buttons
    buttons = [
        [Button.url("Query?", url="https://t.me/ Aspiranthelper836"),
         Button.url("Channel", url="https://t.me/SRC2026_1")]
    ]
...
@aadi.on(events.NewMessage(pattern='/help'))
...
async def youtube_dl_command(_, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            caption = f"{video_info['title']}\n\n__**Powered by [Advance Content Saver Bot](https://t.me/SRC2026_1)**__"  # Set caption to the title of the video
